...
| `{"status":"not_ready","reason":"migrations_pending","pendingCount":21}` 503 | the schema is older than the build | run the migrations: `pnpm db:migrate` from a checkout, or the `migrate` command in [Operations §1](OPERATIONS.md#schema-changes) |
...
| `transportErrors` 0 and `droppedByCause.no_verdict` 0 | with drops, nothing failed: the collector answers, but slower than you record or than `shutdown({ timeoutMs })` waits; or the process shut down before any send had used its three attempts |
...
| `configuration_error` | `setting_unusable` | an optional setting had the wrong type or range, and the default was used; for `redact` or `knownSafeNames` with entries that are not strings (or not key names), or more than 1,000 entries, the usable entries are kept and the rest dropped, while a `redact` that cannot be read at all keeps only the built-in secret names and a `knownSafeNames` that cannot be read exempts nothing; or, for `detail.setting` `deployment.gitCommit`, `deployment.version` or `deployment.image`, that field was not a string, was empty or only whitespace, or was too long, and was left off while the rest of the deployment was sent; `deployment.*` means keys other than those three were left off; `deployment` means events carry no deployment at all | fix the value; nothing is converted or trimmed, so `"5000"` from `process.env` is not a number; `counters().rejectedSettings` lists every name |
