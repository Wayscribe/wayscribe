...
| `configuration_error` | `entity_invalid` | an entity was missing, or its type or id was not a non-empty string; events are filed under `unknown`/`unknown`, and `journeyIdFor` returns a random id | pass `{ type, id }` with non-empty string values |
