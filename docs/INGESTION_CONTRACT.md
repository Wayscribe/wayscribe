...
- **Dry runs that share a journey wait for each other** (ADR-063). Before its
  first event, a dry run takes a lock for every distinct `event.journeyId` its
  elements carry, one at a time in a fixed order, and holds them until its
  rollback; an element without a readable journey id takes none. So two dry
  runs naming the same journeys run one after the other, the second waiting at
  its start, instead of each holding one journey and waiting for the other,
  which answered `storage_error` for events a real send would store. The wait
  ...
  touches one journey. A real event can still wait on a journey a dry run
  holds, as the item above says.
