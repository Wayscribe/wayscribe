...
trace/span IDs to existing trace fields, using lowercase hex. Missing, empty,
all-zero or wrong-length optional IDs are omitted independently, following the
[pinned log schema](https://github.com/open-telemetry/opentelemetry-proto/blob/790608c4d51e6ffc12210b541e8514cbed9e91a4/opentelemetry/proto/logs/v1/logs.proto);
an invalid optional trace does not refuse an otherwise valid business record.
No ID is generated by this receiver: missing event identity is a permanent record
refusal.
...
An absent mapped attribute omits its optional event field. A present attribute
with an absent value message or an empty AnyValue maps to null; a nested map
entry with no value also maps to null. Empty strings, arrays and maps retain
those types. This follows [OTel's empty-value mapping](https://opentelemetry.io/docs/specs/otel/common/attribute-type-mapping/#empty-values)
while making the adapter's absent-message choice explicit. An absent KeyValue
key is the protobuf default empty string, including for duplicate detection.
Typed business fields still reject null when their protocol type requires a
string, number, array or map. String-typed fields, aliases and known error fields
require an original stringValue; bytes or large integers converted to strings
are payload values, not substitutes for typed identity. Numeric event fields
require numeric AnyValue variants and the existing integer/range checks.
Nonfinite doubles anywhere in a mapped value permanently refuse that record;
they must not silently become JSON null. Metadata values are converted JSON
scalars only: null, string, boolean or finite number. Structured data belongs in
input/output. These are adapter restrictions; native metadata is unchanged.
