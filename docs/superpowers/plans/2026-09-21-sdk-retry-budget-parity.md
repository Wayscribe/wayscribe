...
- [ ] **Step 3: Verify adjacent behavior and refresh local artifacts.** Cover cap exhaustion, elapsed exhaustion, expiry-only/no-HTTP cycles, partial success, permanent 4xx, missing verdicts, requeue and stable IDs/bytes using existing deterministic time seams. Run the relevant Node transport tests and Python transport tests, affected package typechecks/lint and Python Ruff. Run Python covering tests on available 3.11–3.14 interpreters. Rebuild the local Node bundle and Python wheel/sdist so they contain the corrected source; prove a focused retry-budget regression from the installed wheel outside the checkout with no source-path fallback. These remain unpublished artifacts. Do not repeat unrelated SDK conformance/API suites unless a change affects their contracts.
