...
- [ ] **Step 1: Write configuration tests and observe RED.** Through `loadServerEnv`, prove blank/missing means disabled, default-only/project-only policies work, overrides do not merge missing fields, 1000 project entries pass and 1001 fail, and every integer boundary is enforced. Reject strings/bools in numeric fields, arrays/null/unknown fields and invalid project IDs. Normalize accepted UUID project keys to lowercase, matching database project IDs; test that an uppercase key applies to its canonical project and that distinct keys colliding after normalization are refused rather than overwritten. Use a distinctive sentinel in malformed input and assert ConfigError names `INGESTION_LIMITS_JSON` without the sentinel. Export the parsed type for API use; keep arbitrary setting content out of errors.
...
- [ ] **Step 3: Implement bounded state and test adversarial churn.** Validate internal event costs as integers 0..100. Refill at `eventsPerMinute / 60000` per elapsed millisecond and cap at burst. A release closure can decrement only its own reservation and only once. Walk a fixed small number of tracked entries per new-project admission using a bounded linked-list cursor, removing only idle fully refilled entries; no interval or per-request timer. Do not retain a Map iterator across mutations: the existing web LoginLimiter documents a measured V8 rehash-retention failure from that pattern. Use the map for lookup and separately owned bounded nodes for incremental traversal. Fill all 10,000 slots with depleted or active entries, refuse a new project with `capacity`, advance time/release and prove incremental reclamation eventually admits new projects without resetting depleted budgets. Exercise long elapsed times, the exact refill boundary, repeated release and map churn. An unlimited project creates no state.
