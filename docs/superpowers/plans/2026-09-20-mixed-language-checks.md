...
- [ ] **Step 1: Write configuration and request-shape tests and observe RED.** Assert help before credential access, exact arity, unknown/duplicate flags, no credentials in parser errors, no URL userinfo/query/fragment, missing explicit settings, invalid alternate env names and ingestion/admin token separation. Reject nonregular/over-limit input files, invalid UTF-8/JSON/public batch-request shape and over-100 event lists before networking. Preview leaves individual event validation to the server's per-position dry-run verdicts and does not re-serialize input. Check validates its generated event through public parseEnvelope. Bound the file using open/fstat plus a bounded read from the same descriptor, not stat-then-unbounded-read.
